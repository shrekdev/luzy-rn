...
import {healthAppointmentsCardData, tabBarWithBack} from '../../helper/appConstant'
...
class AppointmentHome extends Component {
    ...
    renderCard = (data, index) => {
        ...
        return <Card
            ...
            style={{
                ...
                marginRight: index === healthAppointmentsCardData.length - 1 ? wp('45%') : 0
            }}
    ...
    onIconClick = (from) => {
        ...
        } else if (from === 'next' && this.selectedCard < healthAppointmentsCardData.length - 1) {
            this.scrollview.scrollTo({x: (this.selectedCard + 1) * 210, y: 0, animated: true})
        }
    };
    ...
    render() {
        const {container, arrow, indicatorStyle, header,subText} = styles;
        const {safeArea, navigation} = this.props;

        return (
            <View style={[container, {paddingBottom: hp('10%') + safeArea.bottom}]}>
                <ImageBackground style={{height: hp('37%')}} source={{uri: 'prepare_your_food_background'}}>
                    <View style={{marginTop: hp('2.5%'), alignItems: 'center'}}>
                        <Image source={{uri: 'health_appointment_big_icon'}}
                               style={{height: hp('20%'), width: wp('40%')}} resizeMode={'contain'}/>
                        <Text style={{...header, textAlign: 'center'}}>{`APPOINTMENTS`}</Text>
                        <Text style={subText}>
                                {`Here you can make a new appointment,\nand find informations about your\nappointment history.`}
                            </Text>
                    </View>
                </ImageBackground>
                ...
                <ScrollView
                    ...
                    {
                        healthAppointmentsCardData.map((data, index) => this.renderCard(data, index))
                    }
                </ScrollView>
                ...
                <View style={{
                    ...
                    <View style={{flexDirection: 'row'}}>
                        {healthAppointmentsCardData.map((data, index) => this.renderCardBottom(index))}
                    </View>
