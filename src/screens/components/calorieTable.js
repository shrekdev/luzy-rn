...
import {calorieTableCardData, tabBarWithBack} from '../../helper/appConstant';
import RecommendedFood from '../containers/recommendedFood';
import NotRecommendedFood from '../containers/notRecommendedFood';
import {setAsyncStorage} from "../../helper/appHelper";


class CalorieTable extends Component {
    ...
    renderCard = (data, index) => {
        ...
        return <Card
            ...
            style={{
                ...
                marginRight: index === calorieTableCardData.length - 1 ? wp('45%') : 0
            }}
    ...
    onIconClick = (from) => {
        ...
        } else if (from === 'next' && this.selectedCard < calorieTableCardData.length - 1) {
            this.scrollview.scrollTo({x: (this.selectedCard + 1) * 210, y: 0, animated: true})
        }
    };
            ...
            <View style={[container, {paddingBottom: hp('10%') + safeArea.bottom}]}>
                <ImageBackground style={{height: hp('36%')}} source={{uri: 'prepare_your_food_background'}}>
                    <View style={{marginTop: hp('2.5%'), alignItems: 'center'}}>
                        <Image source={{uri: 'food_big_calorie_table'}}
                               style={{height: hp('25%'), width: wp('60%')}} resizeMode={'contain'}/>
                        <Text style={{...header, textAlign: 'center'}}>{'CALORIE TABLE'}</Text>
                    </View>
                </ImageBackground>
                ...
                <ScrollView
                    ...
                    {
                        calorieTableCardData.map((data, index) => this.renderCard(data, index))
                    }
                </ScrollView>
                ...
                <View style={{
                    ...
                    <View style={{flexDirection: 'row'}}>
                        {calorieTableCardData.map((data, index) => this.renderCardBottom(index))}
                    </View>
