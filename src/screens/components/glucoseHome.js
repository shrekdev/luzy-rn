...
import {StyleSheet, ImageBackground, Text, View, Image, TouchableOpacity, ScrollView, Animated, Dimensions} from 'react-native';
import Constant, {normalize} from '../../helper/themeHelper';
import {widthPercentageToDP as wp, heightPercentageToDP as hp} from '../../helper/responsiveScreen';
import {Card, BottomTab} from "./../common";
import {glucoseCardData, tabBarWithBack} from '../../helper/appConstant'

class GlucoseHome extends Component {
    ...
    renderCard = (data, index) => {
        ...
        return <Card
            ...
            title={data.title}
            subtitle={'See More'}
            style={{width: wp('50%'), height: (hp('25%') - safeArea.bottom),
                shadowColor: Constant.color.lightGray, marginRight: index === glucoseCardData.length - 1 ? wp('45%') : 0}}
    ...
    onIconClick = (from) => {
        ...
        } else if(from === 'next' && this.selectedCard < glucoseCardData.length - 1 ){
            this.scrollview.scrollTo({x: (this.selectedCard + 1) * 210 , y: 0, animated: true})
        }
    };
            ...
            <View style={[container,{paddingBottom: hp('10%')+safeArea.bottom}]}>
                    <ImageBackground style={{height: hp('36%')}} source={{uri: 'app_big_blue_background'}}>
                        <View style={{marginTop: hp('2.5%'), alignItems:'center'}}>
                            <Image source={{uri: 'monitoring_glucose_big_monitoring'}} style={{height: hp('20%'),width: wp('50%')}} resizeMode={'stretch'} />
                            <Text style={{...header, textAlign: 'center'}}>{`GLUCOSE\nMONITORING`}</Text>
                        </View>
                    </ImageBackground>
                    <ScrollView
                        ...
                        {
                            glucoseCardData.map((data, index) => this.renderCard(data, index))
                        }
                    </ScrollView>

                    <View style={{...indicatorStyle, marginBottom:hp('2%')
                        ...
                        <View style={{flexDirection: 'row'}}>
                            {glucoseCardData.map((data, index) => this.renderCardBottom(index))}
                        </View>
