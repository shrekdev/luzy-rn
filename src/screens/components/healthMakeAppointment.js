...
import { StyleSheet, ScrollView, Text, View, Image, Modal, Animated, TouchableOpacity } from 'react-native';
...
class HealthMakeAppointment extends Component {
    ...
    changeButtonState = () => {
        ...
        let btnColor = (doctorType !== '' && appointmentTime !== '') && Constant.color.lightblue || Constant.color.lightGray;
    ...
    showModal = () => {
        ...
        return (
                <View style={{ ...styles.modalContainer }}>
                    <View style={{ backgroundColor: Constant.color.lightSky, height: hp('7%'), width: wp('86%'), borderTopLeftRadius: hp('2%'), borderTopRightRadius: hp('2%'), padding: hp('2%'), }}>
                        <TouchableOpacity onPress={() => this.setState({ modalVisible: false })}>
                            <Image style={{ height: hp('3%'), width: wp('4%') }} resizeMode='contain'
                                source={{ uri: 'health_close_pop_up' }} />
                        </TouchableOpacity>
                    </View>
                    <Image style={{
    ...
    showModalOnUnsuccess = () => {
        ...
        return (
                <View style={{ ...styles.modalContainer }}>
                    ...
                    <Image style={{ height: hp('10%'), width: wp('20%'), marginTop: hp('5%') }} resizeMode='contain'
                        source={{ uri: 'unsucces' }} />
                    <Text style={[styles.modal_text_top, { marginTop: hp('1%') }]}>FAIL</Text>
                    <Text style={{
                        ...styles.modal_text_bottom,
                        fontFamily: Constant.font.robotoBold
    ...
    render() {
        ...
        const { container, header, subText, modalView, label, dd_icon, dd_arrow, dd_btn, dd_image, dd_text, dd_options } = styles;
        const { btnColor, currentDateTime, appointmentTime, datePickerText, modalVisible, doctorType } = this.state;
        const { safeArea, navigation } = this.props;
        return (
            <View style={[container, { paddingBottom: hp('10%')}]}>
                ...
                >
                    <View style={{
                        ...style.modalView,
                        marginTop: Platform.OS === 'ios' ? hp('11.5%') : hp('18.5%'),
                        marginBottom: 0
                    }}>
                    {this.showModal()}
                    </View>
