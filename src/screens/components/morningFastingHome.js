...
import MorningFastingActivity from '../containers/morningFastingActivity'
import {tabBarWithBack} from "../../helper/appConstant";
import {getAsyncStorage} from "../../helper/appHelper";

class MorningFastingHome extends Component {
    ...
    onRegister = () => {
        ...
        this.state.btnColor === Constant.color.lightblue ? this.props.navigation.navigate('MorningFastingActivity') : alert('Input glucose value');
    };

    render() {
        const {header, textInput, label, inputContainer, inputIcon} = styles;
        const {btnColor, glucose} = this.state;
        ...
        return (
            <View style={{flex: 1,paddingBottom: hp('10%')+safeArea.bottom}}>
                ...
                >
                    <ScrollView style={{backgroundColor: Constant.color.lightSky, height: hp('72%')}}
                                contentContainerStyle={{paddingBottom: 20}}
                                showsVerticalScrollIndicator={false}>
                        <Image style={{height: hp('41%')}} resizeMode='stretch' source={{uri: 'glucose_morning_fasting_illustration'}}/>
                        <View style={{flex:1, alignItems: 'center'}}>
                            <Text style={[header,{marginTop:hp('4%')}]}>{'MORNING FASTING'}</Text>
